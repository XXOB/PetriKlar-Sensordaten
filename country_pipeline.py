...
def combine(payloads):
    ...
    archive_references = {**read(ROOT / 'austria-level-references.json').get('references', {}),
                          **read(ROOT / 'nl-level-references.json').get('references', {})}
    report = {}
...
def publish():
    ...
    write(ROOT / 'europe-directory.json', {'fetched_at':merged.get('fetched_at'), 'stations':[{'id':s['id'],'items':s.get('items',[])} for s in merged['stations'] if s.get('country') in ('AT','CH','NL')]})
